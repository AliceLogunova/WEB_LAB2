...
public class ControllerServlet extends HttpServlet {
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        boolean isCorrect = validateParam(xStr, yStr, rStr);
    ...
    private boolean validateParam(String xStr, String yStr, String rStr) {
        try {
            double x = Double.parseDouble(xStr);
            double y = Double.parseDouble(yStr);
            double r = Double.parseDouble(rStr);

            boolean correctX = (x >= -3) && (x <= 5);
            boolean correctY = (y >= -3) && (y <= 5);
            boolean correctR = (r >= 1) && (r <= 3);

            return correctX && correctY && correctR;
        }
    ...
    private boolean processPoint(String xStr, String yStr, String rStr) {
        if(validateParam(xStr, yStr, rStr)) {
            double x = Double.parseDouble(xStr);
            double y = Double.parseDouble(yStr);
            double r = Double.parseDouble(rStr);
